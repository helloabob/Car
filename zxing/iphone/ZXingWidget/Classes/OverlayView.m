...
- (id) initWithFrame:(CGRect)theFrame cancelEnabled:(BOOL)isCancelEnabled oneDMode:(BOOL)isOneDModeEnabled showLicense:(BOOL)showLicenseButton {
  ...
  if( self ) {
    ...
    if (self.cancelEnabled) {
        UIButton *butt = [[[UIButton alloc] initWithFrame:CGRectMake(0, 0, 80, 50)]autorelease];
        butt.layer.cornerRadius = 10;
        butt.backgroundColor = [UIColor colorWithRed:0 green:0.6 blue:0 alpha:1];
...
- (void)drawRect:(CGRect)rect {
  ...
  if (displayedMessage == nil) {
      self.displayedMessage=@"请将扫描的二维码至于下面的框内";
